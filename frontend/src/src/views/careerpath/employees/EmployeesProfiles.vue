<template>
    <div class="p-4">
      ...
      <div v-if="loading" class="text-center p-8">
        ...
        <p class="mt-2 text-gray-600">Loading employee profile...</p>
      </div>
  ...
  // Simplified fetch employee function
  async function fetchEmployee() {
    try {
      ...
      const result = await response.json();
      if (result.errors) {
        throw new Error(result.errors[0]?.message || 'Failed to load employee');
      }
  
      // Map employee data to the expected structure
      const employeeData = result.data.employee;
      candidateData.value = {
        id: employeeData.id,
        companyId: employeeData.companyId,
        companyName: employeeData.companyName,
        candidateName: employeeData.name, // Map name to candidateName for compatibility
        ...
        // Set default values for candidate-specific fields
        candidateCV: '',
        recruitmentProcessId: '',
        recruitmentProcessName: employeeData.role || 'Employee', // Use role as process name
  ...
  // Add a watch effect to update the document title when the employee data changes
  watch(() => formattedName.value, (newName) => {
    if (newName) {
      document.title = `${newName} - Employee Profile`;
    } else {
