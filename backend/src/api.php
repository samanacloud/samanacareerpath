...
function isSessionValid() {
    return isset($_SESSION['authenticated']) && (time() - $_SESSION['authenticated'] <= 300); // 300 seconds = 5 minutes
}
...
function handleAuthentication() {
    if (!isSessionValid()) {
        if (!authenticateUser()) {
            ...
            $_SESSION['authenticated'] = time(); // Update the authentication timestamp
        }
    } else {
        // Session is valid, update the timestamp to extend the session
        $_SESSION['authenticated'] = time();
    }
}

// Call the authentication function at the beginning
